...
// Delta requires CONDSTORE; the behavioral round-trips (initial/incremental,
// flag-change detection, UIDVALIDITY reset) run against real Dovecot in
...
func TestDeltaTokenRoundTrip(t *testing.T) {
	// The opaque token round-trips UIDVALIDITY + a 64-bit MODSEQ.
	const uv = uint32(42)
	const ms = uint64(1) << 40 // exceeds 32 bits: MODSEQ must not be truncated
	gotUV, gotMS, err := decodeDeltaToken(encodeDeltaToken(uv, ms))
	if err != nil {
		t.Fatalf("decode: %v", err)
	}
	if gotUV != uv || gotMS != ms {
		t.Errorf("round-trip = (%d, %d), want (%d, %d)", gotUV, gotMS, uv, ms)
	}
}
